...
bool Sock::ws_init() const {
	...
	if(ws_data.wVersion < WS_VERSION) {
		std::cerr << "Incorrect Winsock version. Wanted: < " << WS_VERSION << ", got: " << ws_data.wVersion << std::endl;
		return false;
	}
	done = true;
...
bool Sock::sread(std::string& out, const size_t buffer_length) {
	...
	//end of transmission (return_val == 0) will result in empty string so no need to test explicitly
...
bool Sock::swrite(std::string& msg, const int max_attempts) {
	...
	if(return_val == SOCKET_ERROR) {
		const int last_error = get_error();
		std::cerr << "Socket writing failed. Error code: " << last_error << std::endl;
		return false;
	}
	if(max_attempts < 0) {
...
bool Sock::sdisconnect() {
	...
	if(close(s) != 0) {
		const int last_error = get_error();
		std::cerr << "Error while closing socket. Error code: " << last_error << std::endl;
		return false;
	}
	state = NONE;
	return true;
