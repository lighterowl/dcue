...
void DCue::generate(const std::string& id, const std::string& filename, const bool is_master) {
	std::string json;
	DiscogsReleaseRequest req;
	if(!req.send(id, json, is_master)) {
		std::cerr << "Failed to get valid release info from Discogs (are you connected to the internet? are you sure the ID is correct?)" << std::endl;
		std::exit(1);
	}
	string_map fields;
	...
	if(fields.find("tracklist") != fields.end()) {
		...
		for(std::vector<std::string>::size_type i = 0; i < tracks_size; ++i) {
			...
			if(track_info["position"].find_first_of(".-") != std::string::npos && string_to_numeric<unsigned>(track_info["position"].substr(0, track_info["position"].find_first_of(".-"))) > disc) {
				++disc;
				Disc nd;
				a.discs.push_back(nd);
				track_num = 1;
	...
	//if multidisc album, we have to remove the useless disc we created in the loop
	if(a.discs.size() > 1) {
