...
bool JsonParser::is_pointless(const std::string& s) {
	...
	if(s_size == 1) {
		if(std::isspace(static_cast<int>(s[0])) || !std::isprint(static_cast<int>(s[0]))) {
			return true;
		}
	}
	return false;
...
void JsonParser::try_push_back(std::string text, std::vector<std::string>& results) {
...
std::string::size_type JsonParser::eat_string(const std::string& cur, std::string::size_type pos) {
	++pos;
...
std::string::size_type JsonParser::eat_object(const std::string& cur, std::string::size_type pos, bool array) {
	++pos;
	int embed = 1;
	char open = (array) ? '[' : '{';
	char close = (array) ? ']' : '}';
...
void JsonParser::parse_object(std::string text, string_map& kvs) {
	...
	for(std::vector<std::string>::size_type i = 0; (i + 1) < results_size; i += 2) {
		kvs.insert(std::pair<std::string, std::string>(results[i], results[i + 1]));
	}
}

void JsonParser::break_string_array(std::string& text, std::vector<std::string>& results) {
...
void JsonParser::break_object_array(std::string& text, std::vector<std::string>& results) {
	std::string::size_type found = text.find("{");
	std::string::size_type found_end;
	std::string temp;
	while(found != std::string::npos) {
...
void JsonParser::break_array(std::string& text, std::vector<std::string>& results) {
	prepare_object(text);
	trim(text);
