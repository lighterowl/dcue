...
void Cue::add_generic_time(const unsigned minutes, const unsigned seconds, const unsigned frames) {
...
void Cue::add_index(const std::string& index, const unsigned minutes, const unsigned seconds, const unsigned frames) {
	out += "INDEX ";
	out += index;
	out += " ";
...
void Cue::add_track_index(const unsigned minutes, const unsigned seconds, const unsigned frames) {
...
void Cue::add_pregap_index(const unsigned minutes, const unsigned seconds, const unsigned frames) {
	const std::string index = "00";
	add_index(index, minutes, seconds, frames);
}

void Cue::add_artificial_pregap(const unsigned minutes, const unsigned seconds, const unsigned frames) {
	out += "PREGAP ";
	add_generic_time(minutes, seconds, frames);
	out += LINE_END;
}
...
void CueBuilder::build() {
	...
	for(std::vector<Disc>::size_type i = 0; i < discs_size; ++i) {
		//string sanitisation is really just a way of compensating for the number of dumb cue tools available, double quotes especially confuse them and Medieval CUE Splitter on Windows practically blows up when confronted with backslashes in titles
		//it's not really the application's job to do this (bar perhaps the double quotes) but because there's no actual standard for cue sheets we have to make do and mend
		Cue c;
		...
		time_t cumulative = 0;
		const std::vector<Track>::size_type tracks_size = cuesheet.album.discs[i].tracks.size();
